"""Train a RoBERTa baseline for CoNLL-2003 Named Entity Recognition.
...
class TrainingConfig:
    """Configuration for the baseline RoBERTa NER experiment."""

    model_name: str = "roberta-base"

    dataset_name: str = "conll2003"

    output_dir: Path = Path("models/roberta_ner")

    logging_dir: Path = Path("logs/roberta_ner")
...
def parse_args() -> TrainingConfig:
    ...
    parser = argparse.ArgumentParser(
        description="Train a RoBERTa token classification model on CoNLL-2003."
    )
    ...
    parser.add_argument(
        "--output-dir",
        default="models/roberta_ner"
    )

    parser.add_argument(
        "--logging-dir",
        default="logs/roberta_ner"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Propagate labels to all subword pieces. "
            "By default, only the first piece contributes to the loss."
        ),
    )
...
def load_dataset_data(
    ...
    if not isinstance(dataset, DatasetDict):
        ...
        raise TypeError(
            f"Expected DatasetDict, got {type(dataset)!r}"
        )
...
def tokenize_and_align_labels(
    ...
    for example_index, labels in enumerate(
        examples["ner_tags"]
    ):
...
def train_model(
    ...
    LOGGER.info(
        "Saving model and tokenizer to %s",
        config.output_dir
    )
